  ...
  private async generatePostAndSend(
    ...
    try {
      ...
      const result = await this.postGeneratorService.generatePostImage({
        ...
        productTitle,
        productPrice,
        hasHumanModel: session?.hasHumanModel !== undefined && session?.hasHumanModel !== null ? session.hasHumanModel : (category.id === "moda" ? true : false),
        humanModelGender: session?.modelProfile || (category.id === "moda" ? "woman" : undefined),

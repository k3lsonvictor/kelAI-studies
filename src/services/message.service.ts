  ...
  private async processMessage(incomingDto: IncomingMessageDTO): Promise<void> {
    ...
    if (this.supabaseProfileService) {
      ...
      if (authResult.isAuthorized && authResult.profile) {
        ...
        // Se for um novo usuário Trial recebendo sua primeira mensagem:
        if (userProfile.isTrial && userProfile.isFirstWelcome) {
          ...
          if (userProfile.id) {
            await this.supabaseProfileService.markTrialAsWelcomed(userProfile.id);
          }
          userProfile.isFirstWelcome = false;
        } else {
          console.log(`[MessageService] Remetente ${incomingDto.senderPhone} AUTORIZADO (${userProfile.isTrial ? 'Trial' : 'Supabase Assinante'}). Créditos: ${userProfile.credits}`);
        }
      }
    }
